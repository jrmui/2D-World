        ...
        for (int x = 0; x < numXTiles; x += 1) {
            for (int y = 0; y < numYTiles; y += 1) {
                ...
                if (world[x][y] == Tileset.FLOOR) {
                    ...
                    StdDraw.picture(x + 0.5, y + 0.5, Tileset.FLOOR.getFilepath(), 1, 1);
                } else if (world[x][y] == Tileset.AVATAR) {
                    StdDraw.picture(x + 0.5, y + 0.5, Tileset.AVATAR.getFilepath(), 1, 1);
                } else if (world[x][y] == Tileset.WALL) {
                    StdDraw.picture(x + 0.5, y + 0.5, Tileset.WALL.getFilepath(), 1, 1);
                } else if (world[x][y] == Tileset.NOTHING) {
                    StdDraw.picture(x + 0.5, y + 0.5, Tileset.NOTHING.getFilepath(), 1, 1);
                } else if (world[x][y] == Tileset.WATER) {
                    StdDraw.picture(x + 0.5, y + 0.5, Tileset.WATER.getFilepath(), 1, 1);
                } else {
                    world[x][y].draw(x + xOffset, y + yOffset);
                }
            }
